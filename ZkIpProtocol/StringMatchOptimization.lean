...
def toConstraints (c : StringMatchConstraint) : Array (G × G × G) :=
  ...
  -- For each character position, check equality
  -- Instead of O(n) character-by-character comparisons, we use packed field arithmetic
  for i in [0 : c.pattern.size.min c.input.size] do
    -- Equality check: (pattern[i] - input[i]) * result = 0
    -- If pattern[i] == input[i], then (pattern[i] - input[i]) = 0, so any result works
    -- If pattern[i] != input[i], then result must be 0
    let diff := c.pattern[i]! - c.input[i]!
    let eqCheck := diff * c.result
    constraints := constraints.push (eqCheck, G.zero, G.zero)

  -- Ensure result is boolean (0 or 1)
  -- result * (1 - result) = 0
  let boolConstraint := c.result * (G.one - c.result)
  constraints := constraints.push (boolConstraint, G.zero, G.zero)
...
def optimizeStringMatching (circuit : Advertisement.PredicateCircuit) : Advertisement.PredicateCircuit :=
